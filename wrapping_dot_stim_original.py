...
class WrappingDotStimOriginal(visual.DotStim):
        ...
        if self.fieldShape=='circle':#make more dots than we need and only use those that are within circle 
            while True:#repeat until we have enough 
                ...
                if sum(inCircle)>=nDots: 
                    return numpy.array(new[inCircle,:] [:nDots,:]*self.fieldSize[0]/2.0)
        else:
    ...
    def _update_dotsXY(self): 
        ...
        if self.noiseDots=='walk': 
            ...
            #update signal dots 
            self._dotsXY[self._signalDots,0] +=  self.speed*numpy.reshape(numpy.cos(self._dotsDir[self._signalDots]), (self._signalDots.sum(),)) 
            self._dotsXY[self._signalDots,1] +=  self.speed*numpy.reshape(numpy.sin(self._dotsDir[self._signalDots]), (self._signalDots.sum(),))# 0 radians=East! 
            #update noise dots 
            dead = dead+(~self._signalDots)#just create new ones 
        #handle boundaries of the field 
        if self.fieldShape in  [None, 'square', 'sqr']: 
            self._dotsXY[(self._dotsXY[:,0]>(self.fieldSize[0]/2.0)),0] = numpy.subtract(numpy.mod((self._dotsXY[(self._dotsXY[:, 0]>(self.fieldSize[0]/2.0)),0]), (self.fieldSize[0]/2.0)), (self.fieldSize[0]/ 2.0))
            self._dotsXY[(self._dotsXY[:,1]>(self.fieldSize[0]/2.0)),1] = numpy.subtract(numpy.mod((self._dotsXY[(self._dotsXY[:, 1]>(self.fieldSize[0]/2.0)),1]), (self.fieldSize[0]/2.0)),(self.fieldSize[0]/ 2))
            self._dotsXY[(self._dotsXY[:,0]<-(self.fieldSize[0]/2.0)),0] = numpy.mod(self._dotsXY[(self._dotsXY[:,0]<-(self.fieldSize[0]/2.0)),0], (self.fieldSize[0]/2.0))
            self._dotsXY[(self._dotsXY[:,1]<-(self.fieldSize[0]/2.0)),1] = numpy.mod(self._dotsXY[(self._dotsXY[:,1]<-(self.fieldSize[0]/2.0)),1], (self.fieldSize[0]/2.0))

        elif self.fieldShape == 'circle': 
            #transform to a normalised circle (radius = 1 all around) then to polar coords to check 
            normXY = self._dotsXY/(self.fieldSize[0]/2.0)#the normalised XY position (where radius should be <1)
